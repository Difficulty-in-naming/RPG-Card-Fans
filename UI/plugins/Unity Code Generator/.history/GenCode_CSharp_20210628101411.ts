    ...
    for (let i: number = 0; i < classCnt; i++) {
        ...
        for (let j: number = 0; j < memberCnt; j++) {
            ...
            if (memberInfo.group == 0) {
                if (getMemberByName)
                    writer.writeln('this.%s = this.View.GetChild("%s") as %s;', memberInfo.varName, memberInfo.name, memberInfo.type);
                else
                    writer.writeln('this.%s = this.View.GetChildAt("%s") as %s;', memberInfo.varName, memberInfo.index, memberInfo.type);
            }
            else if (memberInfo.group == 1) {
                if (getMemberByName)
                    writer.writeln('this.%s = this.View.GetController("%s");', memberInfo.varName, memberInfo.name);
                else
                    writer.writeln('this.%s = this.View.GetControllerAt(%s);', memberInfo.varName, memberInfo.index);
            }
            else {
                if (getMemberByName)
                    writer.writeln('this.%s = this.View.GetTransition("%s");', memberInfo.varName, memberInfo.name);
                else
                    writer.writeln('this.%s = this.View.GetTransitionAt(%s);', memberInfo.varName, memberInfo.index);
            }
        }
        writer.endBlock
