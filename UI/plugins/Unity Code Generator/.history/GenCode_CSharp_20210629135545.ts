...
function genCode(handler: FairyEditor.PublishHandler) {
    ...
    for (let i: number = 0; i < classCnt; i++) {
        ...
        writer.startBlock();
        writer.writeln('let url : string = this.Url.toString() + "_" + this.name;');
        ...
        writer.startBlock();
        writer.writeln('let url : string = %s.Url.toString() + "_" + (this).name;',classInfo.className);
        writer.writeln("return UIKit.Inst().Get(url);")
        writer.endBlock();
        writer.writeln("public CloseMySelf()");
        writer.startBlock();
        writer.writeln('let url : string = %s.Url.toString() + "_" + (<any>this).name;',classInfo.className);
