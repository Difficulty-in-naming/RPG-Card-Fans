...
export class BlurWaveEffect extends AbstractEffect{
    ...
    public constructor(private _X:number,private _Y:number,private _Color:Color,private chosenSpeed:number,blend:FairyGUI.BlendMode) {
        super();
        ...
        _Color.G -= Mathf.RandomRange(0, 26);
        _Color.B -= Mathf.RandomRange(0, 51);
        this.Loader.alpha = 0;
    ...
    Update() {
        ...
        if(this._stallTimer < 0) {
            ...
            this.Loader.SetScale(this._scale+ Mathf.RandomRange(-0.08,0.08),this._scale+ Mathf.RandomRange(-0.08,0.08));
            this.Loader.rotation = this._rotation + this._flipper + Mathf.RandomRange(-3,3);
