...
export class View_MainMenuPanel extends UIBase
{
    ...
    public static Url = new ViewInfo("MainMenuPanel","MainMenuPanel")
    ...
    public CloseMySelf()
    {
        let url : string = View_MainMenuPanel.Url.toString() + "." + (<any>this).name;
...
export class View_Button1 extends UIBase
{
    ...
    public static Url = new ViewInfo("MainMenuPanel","Button1")
    public static CreatePanel(...args) : UIBase
    {
        let url : string = this.Url.toString() + "." + this.name;
        let panel : UIBase = UIKit.Inst().Get(url);
    ...
    public CloseMySelf()
    {
        let url : string = View_Button1.Url.toString() + "." + (<any>this).name;
        UIKit.Inst().Destroy(url);
    }
    //不要主动调用这个方法或者修改这个方法
    public Construct()
