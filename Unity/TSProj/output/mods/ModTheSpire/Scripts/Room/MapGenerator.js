...
class MapGenerator {
    ...
    static InitNodes(width, height) {
        let map = new Array();
        for (let y = 0; y < height; y++) {
            let row = new Array();
            for (let x = 0; x < width; x++) {
                row.push(new MapRoomNode_1.MapRoomNode(x, y));
            }
            map.push(row);
        }
        return map;
    }
    static FilterPaths(map) {
    ...
    static CreatePaths(map, pathDensity) {
        let rng = DungeonManager_1.default.Inst.CurrentDungeon.Rng;
        let stage = map[0];
        let fixed = this.GetFixedSizeArray(stage.length);
        let count = Mathf_1.Mathf.Clamp(Mathf_1.Mathf.RandomRange(2, 4, rng), 1, stage.length);
        for (let i = 0; i < fixed.length && count > 0; i++) {
            let index = Mathf_1.Mathf.Floor(Mathf_1.Mathf.RandomRange(0, fixed.length, rng));
    ...
    static Connect(map, node, mustPass) {
        let rng = DungeonManager_1.default.Inst.CurrentDungeon.Rng;
        let nextStage = map[node.Y + 1];
        let successRate = 0.1;
        let pass = false;
        if (nextStage) {
            let connectNodes = new Array();
            for (let i = -1; i <= 1; i++) {
            ...
            for (let i = connectNodes.length - 1; i >= 0; i--) {
                let index = Mathf_1.Mathf.Floor(Mathf_1.Mathf.RandomRange(0, connectNodes.length, rng));
                let isSuccess = 0;
                if (mustPass && i == 0)
                    isSuccess = 0;
                else
                    isSuccess = Mathf_1.Mathf.Random(rng);
    ...
    static GetFixedSizeArray(width) {
    ...
    static IterationAssignRoomType(node, restCount, eliteCount, shopCount) {
        let rng = DungeonManager_1.default.Inst.CurrentDungeon.Rng;
        let type = Mathf_1.Mathf.Random(rng);
        if (node.Y == 6) {
            node.RoomType = MapRoomNode_1.RoomType.CHEST;
        }
        else if (node.Y == 14) {
            node.RoomType = MapRoomNode_1.RoomType.REST;
        }
        else if (node.Y == 0) {
            node.RoomType = MapRoomNode_1.RoomType.MONSTER;
        }
        else {
            if (type > 0.85) {
                node.RoomType = MapRoomNode_1.RoomType.EVENT;
            }
            else if (node.Y >= 5) {
                if (type > 0.8 && eliteCount > 0) {
                    eliteCount--;
                    node.RoomType = MapRoomNode_1.RoomType.ELITE;
                }
                    ...
                    shopCount--;
                    node.RoomType = MapRoomNode_1.RoomType.SHOP;
                }
            }
        }
