...
{
    public static class PuertsLoader_Wrap
    {
            ...
            try
            {

                {
                ...
                
                    {
                        ...
                        var result = new PuertsLoader(Arg0);
                
                        return Puerts.Utils.GetObjectPtr((int)data, typeof(PuertsLoader), result);
        ...
        private static void M_FileExists(IntPtr isolate, IntPtr info, IntPtr self, int paramLen, long data)
        {
            try
            {
                var obj = Puerts.Utils.GetSelf((int)data, self) as PuertsLoader;
        ...
        [Puerts.MonoPInvokeCallback(typeof(Puerts.V8FunctionCallback))]
        private static void G_debugRoot(IntPtr isolate, IntPtr info, IntPtr self, int paramLen, long data)
        {
            try
            {
                var obj = Puerts.Utils.GetSelf((int)data, self) as PuertsLoader;
                var result = obj.debugRoot;
                Puerts.PuertsDLL.ReturnString(isolate, info, result);
            }
            catch (Exception e)
            {
...
{ new Puerts.MethodKey {Name = "ReadFile", IsStatic = false}, M_ReadFile }
                ...
                {
                    {"debugRoot", new Puerts.PropertyRegisterInfo(){ IsStatic = false, Getter = G_debugRoot, Setter = null} }
                },
