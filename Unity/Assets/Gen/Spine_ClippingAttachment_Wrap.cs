...
{
    public static class Spine_ClippingAttachment_Wrap
    {
            ...
            {
                {
                    var argHelper0 = new Puerts.ArgumentHelper((int)data, isolate, info, 0);
                    {
                        ...
                        var result = new Spine.ClippingAttachment(Arg0);
                        return Puerts.Utils.GetObjectPtr((int)data, typeof(Spine.ClippingAttachment), result);
                    }
                }
            }
            catch (Exception e)
        ...
        private static void M_Copy(IntPtr isolate, IntPtr info, IntPtr self, int paramLen, long data)
        {
            try
            {
                var obj = Puerts.Utils.GetSelf((int)data, self) as Spine.ClippingAttachment;
                {
                    {
                        var result = obj.Copy();
        ...
        private static void S_EndSlot(IntPtr isolate, IntPtr info, IntPtr self, int paramLen, long data)
        {
            try
            {
                ...
                obj.EndSlot = argHelper.Get<Spine.SlotData>(false);
            }
            catch (Exception e)
            {
                Puerts.PuertsDLL.ThrowException(isolate, "c# exception:" + e.Message + ",stack:" + e.StackTrace);
