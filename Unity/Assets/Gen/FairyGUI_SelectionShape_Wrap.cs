...
{
    public static class FairyGUI_SelectionShape_Wrap
    {
            ...
            try
            {

                {
            
                    {
                        ...
                        var result = new FairyGUI.SelectionShape();
                
                        return Puerts.Utils.GetObjectPtr((int)data, typeof(FairyGUI.SelectionShape), result);
        ...
        private static void M_Refresh(IntPtr isolate, IntPtr info, IntPtr self, int paramLen, long data)
        {
            try
            {
                var obj = Puerts.Utils.GetSelf((int)data, self) as FairyGUI.SelectionShape;
        ...
        [Puerts.MonoPInvokeCallback(typeof(Puerts.V8FunctionCallback))]
        private static void G_rects(IntPtr isolate, IntPtr info, IntPtr self, int paramLen, long data)
        {
            try
            {
                var obj = Puerts.Utils.GetSelf((int)data, self) as FairyGUI.SelectionShape;
                var result = obj.rects;
                Puerts.ResultHelper.Set((int)data, isolate, info, result);
            }
            catch (Exception e)
            {
...
{"rects", new Puerts.PropertyRegisterInfo(){ IsStatic = false, Getter = G_rects, Setter = null} }
                },
