...
namespace Force.Crc32
{
    ...
    public class Crc32Algorithm: HashAlgorithm
    {
        ...
        /// <returns>Accumulated CRC-32 of all buffers processed so far.</returns>
        public static uint Append(uint initial, byte[] input, int length)
        {
            if (input == null)
            {
            ...
            return AppendInternal(initial, input, 0, length);
        }
        ...
        {
            return Append(0, input, input.Length);
        }
        ...
        /// <summary>
        /// 更加适应Unity的实现方式.这个实现方式可能会增加性能消耗.但是内存占用率会大大降低
        /// </summary>
        ...
        {
            uint hash = 0;
            int blockSize = 4096; //每4096拆分一下,手游一般资源包体大小不大.4096足够用了,速度依然比MD5要快上许多.
            int cbSize = 0;
            using (FileStream fs = new FileStream(path, FileMode.Open, FileAccess.Read))
            {
                var buffer = new byte[blockSize];
                cbSize = fs.Read(buffer, 0, blockSize);
                var interHash = Compute(buffer, 0, cbSize);
                while ((cbSize = fs.Read(buffer, 0, blockSize)) > 0)
                {
                    hash = Append(interHash, buffer, cbSize);
                }
            }
