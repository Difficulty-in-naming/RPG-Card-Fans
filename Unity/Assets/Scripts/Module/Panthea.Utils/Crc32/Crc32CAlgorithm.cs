...
namespace Force.Crc32
{
    ...
    public class Crc32CAlgorithm: HashAlgorithm
    {
        ...
        {
            uint hash = 0;
            int blockSize = 4096; //每200K拆分一下,手游一般资源包体大小不大.200K足够用了
            int cbSize = 0;
            uint interHash;
            using (stream)
            {
                var buffer = new byte[blockSize];
                cbSize = stream.Read(buffer, 0, blockSize);
                interHash = Compute(buffer, 0, cbSize);
                while ((cbSize = stream.Read(buffer, 0, blockSize)) > 0)
                {
