...
public static class MediaManager
{
    ...
    //==================================================================================================================
    // 用文件名播放音效
    public static async UniTask<int> PlayVoice(string path, bool loop = false, float pitch = 1,OnEnd end = null)
    {
        AudioClip clip;
        ...
        if (!temp3 || clip == null)
        {
            clip = await ResKit.LoadAudioClip(path);
    ...
    public static async Task PlayMusic(string name, bool loop = true)
    {
        ...
        {
            clip = await ResKit.LoadAudioClip(name);
            if (clip == null)
            {
                return;
            }
