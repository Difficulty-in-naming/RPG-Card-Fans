...
namespace Panthea.Asset
{
    public class AssetBundleRuntime
    {
        ...
        {
            return await Internal_LoadAssetAsync<T>(path);
        }
        ...
        private async UniTask<T> Internal_LoadAssetAsync<T>(string path) where T : Object
        {
        ...
        private async UniTask<Tuple<T, AssetBundleRequest>> Internal_LoadAssetAsync2<T>(string path) where T : Object
        {
            var ab = await LoadAssetBundleByFilePath(path);
            var asset = await ab.LoadAssetAsync<T>(path);
            mCounter.AddCounter(asset, ab);
        ...
        {
            var tuple = await Internal_LoadAssetAsync2<GameObject>(path);
        ...
        {
            return Internal_LoadAssetSync<T>(path);
        }
        ...
        private T Internal_LoadAssetSync<T>(string path) where T : Object
        {
