...
namespace Panthea.Asset
{
    public interface IAssetsLocator
    {
        ...
        UniTask<GameObject> Instantiate(string filePath, Vector3? position = null, Vector3? rotation = null, Transform parent = null);
        void UnloadAllAssetBundle();
        List<string> GetFilterAssetBundle(string[] path);
        string[] GetDependencies(string abPath);
        Dictionary<string, AssetBundleRequest> GetLoadedAssetBundle();
        string[] GetAllNameInBundle(string abPath);
