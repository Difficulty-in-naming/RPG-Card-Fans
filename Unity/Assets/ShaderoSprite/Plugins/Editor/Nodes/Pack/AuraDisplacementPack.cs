...
{
    [Node(false, "Pack/Aura Displacement Pack")]
    public class AuraDisplacementPack : Node
    {
        [HideInInspector]
        public const string ID = "AuraDisplacementPack";
        [HideInInspector]
        ...
        {
            code = "";
            code += "float4 AuraDisplacementPack(float2 uv,sampler2D source,float x, float y, float value, float motion, float motion2)\n";
            code += "{\n";
            code += "float t = _Time.y;\n";
            code += "float2 mov = uv + (float2(x * t, y * t) * motion);\n";
            code += "float2 mov2 = uv + (float2(x * t * 1.5, y * t * 1.5)*3.5 * motion2);\n";
            code += "mov = lerp(uv, mov, value);\n";
            code += "mov2 = lerp(uv, mov2, value);\n";
            code += "float4 rgba = tex2D(source, mov);\n";
            code += "float4 rgba2 = tex2D(source, mov2);\n";
            code += "rgba.rgb = rgba.rgb * rgba2.rgb*8*value;\n";
            code += "return saturate(rgba);\n";
            code += "}\n";
        }
        ...
        public override Node Create(Vector2 pos)
        {
            ...
            AuraDisplacementPack node = ScriptableObject.CreateInstance<AuraDisplacementPack>();
            node.name = "Aura Displacement Pack";
        ...
        public override bool Calculate()
        {
            ...
            string DefaultName = "_AuraDisplacementPack_" + NodeCount;
            string SourceName = "AuraDisplacementPack_" + NodeCount;
            string DefaultNameVariable1 = "_AuraDisplacementPack_ValueX_" + NodeCount;
            string DefaultNameVariable2 = "_AuraDisplacementPack_ValueY_" + NodeCount;
            string DefaultNameVariable3 = "_AuraDisplacementPack_Size_" + NodeCount;
            ...
            if (AddParameters)
            {
                s_out.ValueLine = "float4 " + DefaultName + " = AuraDisplacementPack(" + uv + "," + Source + "," + DefaultNameVariable1 + "," + DefaultNameVariable2 + "," + DefaultNameVariable3 + "," + m1 + "," + m2 + ");\n";
            }
            else
            {
                s_out.ValueLine = "float4 " + DefaultName + " = AuraDisplacementPack(" + uv + "," + Source + "," + Variable.ToString() + "," + Variable2.ToString() + "," + Variable3.ToString() + "," + m1 + "," + m2 + ");\n";
            }
