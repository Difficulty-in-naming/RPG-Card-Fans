...
{
[Node(false, "UV/FX (UV)/Sliced Vertical Bar UV")]

public class SlicedVerticalBarUV : Node
{
    public const string ID = "SlicedVerticalBarUV";
    ...
    {
            code = "";
            code += "float2 SlicedVerticalBarUV(float2 uv, float b1, float s)\n";
            code += "{\n";
            code += "float ov = uv.y;\n";
            code += "float muv =uv.y;\n";
            code += "muv *= s;\n";
            ...
            code += "muv -= z;\n";
            code += "uv.y = muv / (1. - (z * 2.));\n";
            code += "if (ov < z) { uv.y = s1; }\n";
            code += "if (ov > 1. - z) { uv.y = s2; }\n";
            code += "return uv;\n";
            code += "}\n";
    }
    ...
    protected internal override void NodeGUI()
    {
        ...
        GUILayout.Label("Strech Size: (0.2 to 8f) " + Variable2.ToString("0.00"));
    ...
    public override bool Calculate()
    {
        ...
        string DefaultName = "SlicedVerticalBarUV_";
        ...
        string VoidName = "SlicedVerticalBarUV";
