...
{
[Node(false, "UV/Math (UV)/Lerp UV")]
public class LerpUV : Node
{
    [HideInInspector]
    public const string ID = "LerpUV";
    [HideInInspector]
    ...
    {
            LerpUV node = ScriptableObject.CreateInstance<LerpUV>();

        node.name = "Lerp = A with B";
    ...
    {
        Texture2D preview = ResourceManager.LoadTexture("Textures/previews/nid_uvlerp.jpg");
    ...
    public override bool Calculate()
    {
        ...
        string DefaultName = "LerpUV_" + NodeCount;
        string DefaultNameVariable1 = "_LerpUV_Fade_" + NodeCount;
        ...
        if (parametersOK)
        {
            s_out.ValueLine = "float2 " + DefaultName + "=" + PreviewVariable + ";\n" + DefaultName + " = lerp(" + PreviewVariable + "," + PreviewVariable2 + "," + DefaultNameVariable1 + ");\n";
        }
        else
        {
            s_out.ValueLine = "float2 " + DefaultName + "=" + PreviewVariable + ";\n" + DefaultName + " = lerp(" + PreviewVariable + "," + PreviewVariable2 + "," + Value1 + ");\n";
        }
