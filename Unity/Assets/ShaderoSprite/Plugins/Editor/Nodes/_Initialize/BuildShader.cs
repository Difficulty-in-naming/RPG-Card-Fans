...
namespace _ShaderoShaderEditorFramework
{
    ...
    public class BuildShader : Node
    {
        ...
        private static bool AddTimerParameter;
        private static bool AddUIRectMaskSupport;
        ...
        public override Node Create(Vector2 pos)
        {
            ...
            node.rect = new Rect(pos.x, pos.y, 260, 400);
        ...
        public static string WriteFunction(string AddingTag)
        {
            ...
            if (PageCurl.tag) AddingTag += PageCurl.code;
            if (CircleFade.tag) AddingTag += CircleFade.code;
            if (PolarCoordinatesUV.tag) AddingTag += PolarCoordinatesUV.code;
            if (ColorGradients2.tag) AddingTag += ColorGradients2.code;
            if (FourGradients.tag) AddingTag += FourGradients.code;
            if (SlicedHorizontalBarUV.tag) AddingTag += SlicedHorizontalBarUV.code;
            if (SlicedVerticalBarUV.tag) AddingTag += SlicedVerticalBarUV.code;
            if (ThresholdSmooth.tag) AddingTag += ThresholdSmooth.code;
            if (TiltUpUV.tag) AddingTag += TiltUpUV.code;
            if (SimpleDisplacementRotativeUV.tag) AddingTag += SimpleDisplacementRotativeUV.code;
            ...
            if (DamageIncrustation.tag) AddingTag += DamageIncrustation.code;
            if (DamageIncrustationTint.tag) AddingTag += DamageIncrustationTint.code;
            ...
            if (Generate_Simple3DSphere.tag) AddingTag += Generate_Simple3DSphere.code;
            if (Generate_3DSphere.tag) AddingTag += Generate_3DSphere.code;
            if (Generate_3DPlanet.tag) AddingTag += Generate_3DPlanet.code;
            ...
            if (AuraDisplacementPack.tag) AddingTag += AuraDisplacementPack.code;
            ...
            if (AnimatedShakeUV.tag) AddingTag += AnimatedShakeUV.code;
            if (WorldGrass.tag) AddingTag += WorldGrass.code;
            if (WorldGrassMask.tag) AddingTag += WorldGrassMask.code;
            if (Palette4Swapping.tag) AddingTag += Palette4Swapping.code;
            if (PaletteSwapping2Texture.tag) AddingTag += PaletteSwapping2Texture.code;
            return AddingTag;
        }
        ...
        public static string BuildVertexShader(string shadername, string preview, string res, string ParametersLines, string ParametersDeclarationLines)
        {
            ...
            string First = "";
            First += "//////////////////////////////////////////////////////////////\n";
            First += "/// Shadero Sprite: Sprite Shader Editor - by VETASOFT 2020 //\n";
            First += "/// Shader generate with Shadero 1.9.9                      //\n";
            ...
            string Second = "";
            if (AddTimerParameter) First += "_SetTimer(\"_SetTimer\", Range(0, 1000)) = 0\n";
            ...
            if (AddUIRectMaskSupport) Second += "#include \"UnityUI.cginc\"\n";
            Second += "\n";
            ...
            if (AddUIRectMaskSupport) Second += "float4 worldPosition : TEXCOORD3;\n";
            ...
            if (AddTimerParameter) Second += "float _SetTimer;\n";
            if (AddUIRectMaskSupport) Second += "float4 _ClipRect;\n";
            ...
            Thirth += "{\n";
            Thirth += "v2f OUT;\n";
            if (AddUIRectMaskSupport) Thirth += "OUT.worldPosition = IN.vertex;\n";

            if (AddUIRectMaskSupport)
            {
                Thirth += "OUT.vertex = UnityObjectToClipPos(OUT.worldPosition);\n";
            }
            else
            {
            ...
            if (AddTimerParameter)
            {
                result = Regex.Replace(result, @"\b_Time.\w\b", "_SetTimer");
                result = Regex.Replace(result, @"\b_Time\b", "_SetTimer");
            }
