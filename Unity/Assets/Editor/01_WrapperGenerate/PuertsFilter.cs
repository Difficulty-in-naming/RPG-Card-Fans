    ...
    public static List<List<string>> BlackList
    {
        get
        {
            return new List<List<string>>()  {
                ...
                //TMP
                new List<string>(){"TMPro.TMPro_ExtensionMethods","FindInstanceID","System.Collections.Generic.List`1[T]","T"}
            };
        }
    }
    ...
    static bool Filter(MemberInfo memberInfo)
    {
        ...
        if (blacklist.TryGetValue(fullname, out methodOrProp) && methodOrProp.TryGetValue(memberInfo.Name, out paramtersList))
        {
            ...
            catch
            {
                ...
                foreach (var pInfo in ((MethodInfo)memberInfo).GetParameters()) 
                    list.Add((pInfo.ParameterType.FullName??pInfo.ParameterType.ToString()).Replace("+", "."));

                var m_paramters = list.ToArray();
                foreach (var paramters in paramtersList)
                {
                    //*(通配) / 屏蔽所有此名称的方法
                    if (paramters.Length == 1 && paramters[0] == "*")
